...
pub fn config(cfg: &mut ServiceConfig) {
  cfg.service(scope("session").service(list).service(delete).service(refresh));
}
...
pub async fn get_session_metadata(
  ...
  let country = req.headers().get("cf-ipcountry").and_then(|x| x.to_str().ok());
...
pub async fn issue_session(
  req: HttpRequest, user_id: UserId,
...
#[delete("{id}")]
pub async fn delete(
  info: web::Path<(String,)>, req: HttpRequest, pool: Data<MySqlPool>,
...
pub async fn refresh(
  ...
  let current_user = get_user_from_headers(
    &req, &**pool, &redis, &session_queue, None,
  )
  .await?
  .1;
  let session =
    req.headers().get(AUTHORIZATION).and_then(|x| x.to_str().ok()).ok_or_else(
  ...
  if let Some(session) = session {
    ...
    let new_session = issue_session(
      req, session.user_id, &mut transaction, &redis,
    )
    .await?;
    DBSession::clear_cache(
      vec![(
