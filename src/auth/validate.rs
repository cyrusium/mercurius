...
pub async fn get_user_from_headers<'a, E>(
  req: &HttpRequest, executor: E, redis: &deadpool_redis::Pool,
  session_queue: &AuthQueue, required_scopes: Option<&[Scopes]>,
  ...
  E: sqlx::Executor<'a, Database = sqlx::MySql> + Copy,
{
  ...
  let (scopes, db_user) = get_user_record_from_bearer_token(
    req, None, executor, redis, session_queue,
  )
  .await?
...
pub async fn get_user_record_from_bearer_token<'a, 'b, E>(
  req: &HttpRequest, token: Option<&str>, executor: E,
  redis: &deadpool_redis::Pool, session_queue: &AuthQueue,
  ...
  E: sqlx::Executor<'a, Database = sqlx::MySql> + Copy,
{
  ...
  let possible_user = match token.split_once('_') {
    | Some(("mrp", _)) => {
      let pat = crate::database::models::pat_item::PersonalAccessToken::get(
  ...
  E: sqlx::Executor<'a, Database = sqlx::MySql> + Copy,
{
  let user = get_user_from_headers(
    req, executor, redis, session_queue, None,
  )
  .await?
  .1;
