use crate::auth::{
  email::send_email, get_user_from_headers, session::issue_session,
  validate::get_user_record_from_bearer_token, AuthenticationError,
};
...
impl AuthProvider {
  ...
  pub async fn get_token(
    &self, query: HashMap<String, String>,
  ) -> Result<String, AuthenticationError> {
    let redirect_uri = format!(
      "{}/v2/auth/callback",
      dotenvy::var("SELF_ADDR")?
    );
    ...
    E: sqlx::Executor<'a, Database = sqlx::MySql>,
  {
    Ok(match self {
      ...
      | AuthProvider::Microsoft => {
        let value =
          sqlx::query!("SELECT id FROM users WHERE microsoft_id = $1", id)
            .fetch_optional(executor)
            .await?;
      ...
      | AuthProvider::Google => {
        let value =
          sqlx::query!("SELECT id FROM users WHERE google_id = $1", id)
            .fetch_optional(executor)
            .await?;
  ...
  pub async fn update_user_id(
    &self, user_id: crate::database::models::UserId, id: Option<&str>,
...
pub async fn auth_callback(
  req: HttpRequest, Query(query): Query<HashMap<String, String>>,
  client: Data<MySqlPool>, file_host: Data<Arc<dyn FileHost + Send + Sync>>,
  redis: Data<deadpool_redis::Pool>,
) -> Result<HttpResponse, AuthenticationError> {
  let state = query
    .get("state")
  ...
  }) = flow
  {
    ...
    if let Some(id) = user_id {
      ...
      let user =
        crate::database::models::user::User::get_id(id, &**client, &redis)
          .await?;
      ...
      crate::database::models::user::User::clear_caches(&[(id, None)], &redis)
        .await?;
      ...
      let user_id = if let Some(user_id) = user_id_opt {
        let user = crate::database::models::user::User::get_id(
        ...
        if let Some(email) = &oauth_user.email {
          if crate::database::models::user::User::get_email(email, &**client)
            .await?
            .is_some()
          {
            return Err(AuthenticationError::DuplicateUser);
        ...
        while username.is_none() {
          ...
          let new_id = crate::database::models::user::User::get(
            &test_username, &**client, &redis,
          )
          .await?;
        ...
        if let Some(username) = username {
          crate::database::models::user::User {
            id: user_id,
...
pub async fn delete_auth_provider(
  req: HttpRequest, pool: Data<MySqlPool>, redis: Data<deadpool_redis::Pool>,
  ...
  crate::database::models::user::User::clear_caches(
    &[(user.id.into(), None)],
    &redis,
  )
  .await?;
...
pub async fn create_account_with_password(
  ...
  if crate::database::models::user::User::get(
    &new_account.username, &**pool, &redis,
  )
  .await?
  .is_some()
  {
  ...
  let password_hash =
    hasher.hash_password(new_account.password.as_bytes(), &salt)?.to_string();

  if crate::database::models::user::User::get_email(&new_account.email, &**pool)
    .await?
    .is_some()
  {
    return Err(ApiError::InvalidInput(
...
pub async fn login_password(
  ...
  let user = if let Some(user) =
    crate::database::models::user::User::get(&login.username, &**pool, &redis)
      .await?
  {
    user
  } else {
    let user =
      crate::database::models::user::User::get_email(&login.username, &**pool)
        .await?
        .ok_or_else(|| AuthenticationError::InvalidCredentials)?;

    crate::database::models::user::User::get_id(user, &**pool, &redis)
      .await?
      .ok_or_else(|| AuthenticationError::InvalidCredentials)?
  };
...
async fn validate_2fa_code(
  input: String, secret: String, allow_backup: bool,
  user_id: crate::database::models::UserId, redis: &deadpool_redis::Pool,
  pool: &MySqlPool, transaction: &mut sqlx::Transaction<'_, sqlx::MySql>,
  ...
  if input == token {
    ...
    let backup_codes =
      crate::database::models::user::User::get_backup_codes(user_id, pool)
        .await?;
...
pub async fn login_2fa(
  ...
  if let Flow::Login2FA {
    user_id,
  } = flow
  {
    let user =
      crate::database::models::user::User::get_id(user_id, &**pool, &redis)
        .await?
        .ok_or_else(|| AuthenticationError::InvalidCredentials)?;
...
pub async fn finish_2fa_flow(
  req: HttpRequest, pool: Data<MySqlPool>, redis: Data<deadpool_redis::Pool>,
  login: web::Json<Login2FA>, session_queue: Data<AuthQueue>,
) -> Result<HttpResponse, ApiError> {
  let flow = Flow::get(&login.flow, &redis)
    .await?
    .ok_or_else(|| AuthenticationError::InvalidCredentials)?;

  if let Flow::Initialize2FA {
    user_id,
    secret,
  } = flow
  {
    ...
    sqlx::query!(
      ...
      WHERE (id = $2)
      ",
      secret, user_id as crate::database::models::ids::UserId,
    )
    .execute(&mut *transaction)
    .await?;
...
pub async fn remove_2fa(
  req: HttpRequest, pool: Data<MySqlPool>, redis: Data<deadpool_redis::Pool>,
  login: web::Json<Remove2FA>, session_queue: Data<AuthQueue>,
) -> Result<HttpResponse, ApiError> {
  let (scopes, user) = get_user_record_from_bearer_token(
    &req, None, &**pool, &redis, &session_queue,
  )
  .await?
  .ok_or_else(|| AuthenticationError::InvalidCredentials)?;
  ...
  crate::database::models::user::User::clear_caches(&[(user.id, None)], &redis)
    .await?;
  transaction.commit().await?;
...
pub async fn reset_password_begin(
  ...
  let user = if let Some(user_id) =
    crate::database::models::user::User::get_email(
    ...
    .await?
  {
    ...
    crate::database::models::user::User::get(
      &reset_password.username, &**pool, &redis,
    )
    .await?
  };
...
pub async fn change_password(
  req: HttpRequest, pool: Data<MySqlPool>, redis: Data<deadpool_redis::Pool>,
  change_password: web::Json<ChangePassword>, session_queue: Data<AuthQueue>,
) -> Result<HttpResponse, ApiError> {
  let user = if let Some(flow) = &change_password.flow {
    let flow = Flow::get(flow, &redis).await?;

    if let Some(Flow::ForgotPassword {
      user_id,
    }) = flow
    {
      let user =
    ...
    &change_password.new_password
  {
    ...
    let salt = SaltString::generate(&mut ChaCha20Rng::from_entropy());
    let password_hash =
      hasher.hash_password(new_password.as_bytes(), &salt)?.to_string();
  ...
  sqlx::query!(
    ...
    WHERE (id = $2)
    ",
    update_password, user.id as crate::database::models::ids::UserId,
  )
  .execute(&mut *transaction)
  .await?;
...
pub async fn set_email(
  req: HttpRequest, pool: Data<MySqlPool>, redis: Data<deadpool_redis::Pool>,
  email: web::Json<SetEmail>, session_queue: Data<AuthQueue>,
) -> Result<HttpResponse, ApiError> {
...
pub async fn resend_verify_email(
  ...
  if let Some(email) = user.email {
    ...
    send_email_verify(
      email, flow, "We need to verify your email address.",
    )?;
...
pub async fn verify_email(
  pool: Data<MySqlPool>, redis: Data<deadpool_redis::Pool>,
  ...
  }) = flow
  {
    ...
    if user.email != Some(confirm_email) {
      return Err(ApiError::InvalidInput(
        "E-mail does not match verify email. Try re-requesting the verification link.".to_string(),
      ));
    }
...
fn send_email_verify(
  email: String, flow: String, opener: &str,
