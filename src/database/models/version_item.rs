...
impl Version {
  ...
  pub async fn remove_full(
    id: VersionId, redis: &deadpool_redis::Pool,
    transaction: &mut sqlx::Transaction<'_, sqlx::MySql>,
  ...
  pub async fn get<'a, 'b, E>(
    id: VersionId, executor: E, redis: &deadpool_redis::Pool,
  ) -> Result<Option<QueryVersion>, DatabaseError>
  where
    E: sqlx::Executor<'a, Database = sqlx::MySql>,
  {
    Self::get_many(&[id], executor, redis).await.map(|x| x.into_iter().next())
  }

  pub async fn get_many<'a, E>(
    version_ids: &[VersionId], exec: E, redis: &deadpool_redis::Pool,
  ...
  pub async fn get_file_from_hash<'a, 'b, E>(
    algo: String, hash: String, version_id: Option<VersionId>, executor: E,
    redis: &deadpool_redis::Pool,
  ) -> Result<Option<SingleFile>, DatabaseError>
  where
    E: sqlx::Executor<'a, Database = sqlx::MySql> + Copy,
  {
    Self::get_files_from_hash(algo, &[hash], executor, redis).await.map(|x| {
      x.into_iter().find_or_first(|x| Some(x.version_id) == version_id)
    })
  }

  pub async fn get_files_from_hash<'a, 'b, E>(
    algorithm: String, hashes: &[String], executor: E,
    redis: &deadpool_redis::Pool,
      ...
      .arg(
        file_ids_parsed
          ...
          .map(|hash| {
            format!(
              "{}:{}_{}",
              VERSION_FILES_NAMESPACE, algorithm, hash
            )
          })
          .collect::<Vec<_>>(),
      )
  ...
  pub async fn clear_cache(
    version: &QueryVersion, redis: &deadpool_redis::Pool,
  ) -> Result<(), DatabaseError> {
    let mut redis = redis.get().await?;
    ...
    for file in &version.files {
      for (algo, hash) in &file.hashes {
        cmd.arg(format!(
          "{}:{}_{}",
          VERSION_FILES_NAMESPACE, algo, hash
        ));
      }
    }
