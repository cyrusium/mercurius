...
impl Project {
  ...
  pub async fn remove(
    id: ProjectId, transaction: &mut sqlx::Transaction<'_, sqlx::MySql>,
  ...
  pub async fn get_id<'a, 'b, E>(
    id: ProjectId, executor: E, redis: &deadpool_redis::Pool,
  ) -> Result<Option<QueryProject>, DatabaseError>
  where
    E: sqlx::Executor<'a, Database = sqlx::MySql>,
  {
  ...
  pub async fn get_many_ids<'a, E>(
    project_ids: &[ProjectId], exec: E, redis: &deadpool_redis::Pool,
  ...
  pub async fn get_many<'a, E, T: ToString>(
    project_strings: &[T], exec: E, redis: &deadpool_redis::Pool,
  ) -> Result<Vec<QueryProject>, DatabaseError>
  where
    E: sqlx::Executor<'a, Database = sqlx::MySql>,
  {
    ...
    let mut remaining_strings =
      project_strings.iter().map(|x| x.to_string()).collect::<Vec<_>>();
  ...
  pub async fn get_dependencies<'a, E>(
    id: ProjectId, exec: E, redis: &deadpool_redis::Pool,
  ) -> Result<
    Vec<(
      Option<VersionId>,
      Option<ProjectId>,
    ...
    E: sqlx::Executor<'a, Database = sqlx::MySql>,
  {
    ...
    let dependencies = cmd("GET")
      .arg(format!(
        "{}:{}",
        PROJECTS_DEPENDENCIES_NAMESPACE, id.0
      ))
  ...
  pub async fn clear_cache(
    id: ProjectId, slug: Option<String>, clear_dependencies: Option<bool>,
    redis: &deadpool_redis::Pool,
  ) -> Result<(), DatabaseError> {
    let mut redis = redis.get().await?;
    let mut cmd = cmd("DEL");
