...
impl Session {
  pub async fn get<'a, E, T: ToString>(
    id: T, exec: E, redis: &deadpool_redis::Pool,
  ) -> Result<Option<Session>, DatabaseError>
  where
    E: sqlx::Executor<'a, Database = sqlx::MySql>,
  {
    Self::get_many(&[id], exec, redis).await.map(|x| x.into_iter().next())
  }

  pub async fn get_id<'a, 'b, E>(
    id: SessionId, executor: E, redis: &deadpool_redis::Pool,
  ) -> Result<Option<Session>, DatabaseError>
  where
    E: sqlx::Executor<'a, Database = sqlx::MySql>,
  {
  ...
  pub async fn get_many_ids<'a, E>(
    session_ids: &[SessionId], exec: E, redis: &deadpool_redis::Pool,
  ...
  pub async fn get_many<'a, E, T: ToString>(
    session_strings: &[T], exec: E, redis: &deadpool_redis::Pool,
  ) -> Result<Vec<Session>, DatabaseError>
  where
    E: sqlx::Executor<'a, Database = sqlx::MySql>,
  {
    ...
    let mut remaining_strings =
      session_strings.iter().map(|x| x.to_string()).collect::<Vec<_>>();
      ...
      for session in sessions {
          ...
          session.and_then(|x| serde_json::from_str::<Session>(&x).ok())
        {
          remaining_strings.retain(|x| {
            &to_base62(session.id.0 as u64) != x && &session.session != x
          });
    ...
    if !remaining_strings.is_empty() {
      ...
      for session in db_sessions {
        cmd("SET")
          .arg(format!(
            "{}:{}",
            SESSIONS_NAMESPACE, session.id.0
          ))
        ...
        cmd("SET")
          .arg(format!(
            "{}:{}",
            SESSIONS_IDS_NAMESPACE, session.session
          ))
          .arg(session.id.0)
          .arg("EX")
          .arg(DEFAULT_EXPIRY)
  ...
  pub async fn get_user_sessions<'a, E>(
    user_id: UserId, exec: E, redis: &deadpool_redis::Pool,
    ...
    E: sqlx::Executor<'a, Database = sqlx::MySql>,
  {
    ...
    let res = cmd("GET")
      .arg(format!(
        "{}:{}",
        SESSIONS_USERS_NAMESPACE, user_id.0
      ))
  ...
  pub async fn remove(
    id: SessionId, transaction: &mut sqlx::Transaction<'_, sqlx::MySql>,
