...
impl User {
  pub async fn insert(
    &self, transaction: &mut sqlx::Transaction<'_, sqlx::MySql>,
  ...
  pub async fn get<'a, 'b, E>(
    string: &str, executor: E, redis: &deadpool_redis::Pool,
  ...
  pub async fn get_id<'a, 'b, E>(
    id: UserId, executor: E, redis: &deadpool_redis::Pool,
  ) -> Result<Option<User>, DatabaseError>
  where
    E: sqlx::Executor<'a, Database = sqlx::MySql>,
  {
  ...
  pub async fn get_many_ids<'a, E>(
    user_ids: &[UserId], exec: E, redis: &deadpool_redis::Pool,
  ...
  pub async fn get_many<'a, E, T: ToString>(
    users_strings: &[T], exec: E, redis: &deadpool_redis::Pool,
  ) -> Result<Vec<User>, DatabaseError>
  where
    E: sqlx::Executor<'a, Database = sqlx::MySql>,
  {
    ...
    let mut remaining_strings =
      users_strings.iter().map(|x| x.to_string()).collect::<Vec<_>>();
    ...
    if !user_ids.is_empty() {
      ...
      for user in users {
        if let Some(user) =
          user.and_then(|x| serde_json::from_str::<User>(&x).ok())
        {
          remaining_strings.retain(|x| {
            &to_base62(user.id.0 as u64) != x && &user.username != x
          });
  ...
  where
    E: sqlx::Executor<'a, Database = sqlx::MySql> + Copy,
  {
    ...
    E: sqlx::Executor<'a, Database = sqlx::MySql> + Copy,
  {
    ...
    .fetch_many(exec)
    .try_filter_map(|e| async {
      Ok(e.right().map(|m| to_base62(m.code as u64)))
    })
  ...
  pub async fn clear_caches(
    user_ids: &[(UserId, Option<String>)], redis: &deadpool_redis::Pool,
    ...
    if let Some(delete_user) = user {
      ...
      if full {
        ...
        for project_id in projects {
          let _result = super::project_item::Project::remove(
            project_id, transaction, redis,
          )
          .await?;
        }
      } else {
