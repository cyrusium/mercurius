...
impl Category {
  ...
  pub async fn get_id_project<'a, E>(
    name: &str, project_type: ProjectTypeId, exec: E,
  ) -> Result<Option<CategoryId>, DatabaseError>
  where
    E: sqlx::Executor<'a, Database = sqlx::MySql>,
  {
...
impl GameVersion {
  ...
  pub async fn list_filter<'a, E>(
    version_type_option: Option<&str>, major_option: Option<bool>, exec: E,
...
impl<'a> GameVersionBuilder<'a> {
  ...
  where
    E: sqlx::Executor<'b, Database = sqlx::MySql>,
  {
