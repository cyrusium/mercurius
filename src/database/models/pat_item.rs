...
impl PersonalAccessToken {
  ...
  pub async fn get_many_ids<'a, E>(
    pat_ids: &[PatId], exec: E, redis: &deadpool_redis::Pool,
  ...
  pub async fn get_many<'a, E, T: ToString>(
    pat_strings: &[T], exec: E, redis: &deadpool_redis::Pool,
  ) -> Result<Vec<PersonalAccessToken>, DatabaseError>
  where
    E: sqlx::Executor<'a, Database = sqlx::MySql>,
  {
    ...
    let mut remaining_strings =
      pat_strings.iter().map(|x| x.to_string()).collect::<Vec<_>>();
      ...
      for pat in pats {
          ...
          pat.and_then(|x| serde_json::from_str::<PersonalAccessToken>(&x).ok())
        {
          remaining_strings.retain(|x| {
            &to_base62(pat.id.0 as u64) != x && &pat.access_token != x
          });
          found_pats.push(pat);
          continue;
        }
    ...
    if !remaining_strings.is_empty() {
      ...
      for pat in db_pats {
        ...
        cmd("SET")
          .arg(format!(
            "{}:{}",
            PATS_TOKENS_NAMESPACE, pat.access_token
          ))
          .arg(pat.id.0)
          .arg("EX")
          .arg(DEFAULT_EXPIRY)
    ...
    E: sqlx::Executor<'a, Database = sqlx::MySql>,
  {
    ...
    let res = cmd("GET")
      .arg(format!(
        "{}:{}",
        PATS_USERS_NAMESPACE, user_id.0
      ))
  ...
  pub async fn remove(
    id: PatId, transaction: &mut sqlx::Transaction<'_, sqlx::MySql>,
