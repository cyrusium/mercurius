...
const PLUGIN_LOADERS: &[&str] = &[
  "bukkit", "spigot", "paper", "purpur", "bungeecord", "waterfall", "velocity",
  "sponge",
];

pub async fn send_discord_webhook(
  project_id: ProjectId, pool: &MySqlPool, webhook_url: String,
  ...
  if let Some(project) = row {
    ...
    client
      ...
      .json(&DiscordWebhook {
        avatar_url: Some(
          "https://cdn.modrinth.com/Modrinth_Dark_Logo.png".to_string(),
        ),
...
fn get_gv_range(
  mut game_versions: Vec<GameVersion>, mut all_game_versions: Vec<GameVersion>,
) -> String {
  ...
  for (i, current_version) in game_versions.iter().enumerate() {
    ...
    if i == 0 {
      ...
      if ((index as i32) - (interval_base[interval_base.len() - 1][1] as i32)
        == 1
