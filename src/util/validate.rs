...
pub fn validation_errors_to_string(
  errors: ValidationErrors, adder: Option<String>,
) -> String {
  ...
  if let Some(field) = key_option {
    if let Some(error) = map.get(field) {
      return match error {
        | ValidationErrorsKind::Struct(errors) => validation_errors_to_string(
          *errors.clone(),
        ...
        | ValidationErrorsKind::List(list) => {
          if let Some((index, errors)) = list.iter().next() {
            output.push_str(&validation_errors_to_string(
              *errors.clone(),
              Some(format!(
                "of list {field} with index {index}"
              )),
            ));
          }
...
pub fn validate_deps(
  ...
  if values
    ...
    .duplicates_by(|x| {
      format!(
        "{}-{}-{}",
        x.version_id.unwrap_or(crate::models::projects::VersionId(0)),
        x.project_id.unwrap_or(crate::models::projects::ProjectId(0)),
